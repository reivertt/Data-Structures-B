...
private:
    ...
    AVLNode* _rightRotate(AVLNode* pivotNode){
        // cout << pivotNode->data << " right rotate" << endl;
    ...
    AVLNode* _leftRotate(AVLNode* pivotNode) {
        // cout << pivotNode->data << " left rotate" << endl;
    ...
    AVLNode* _leftRightCaseRotate(AVLNode* node) {
        // cout << node->data << " lr case" << endl;
        node->left = _leftRotate(node->left);
        return _rightRotate(node);
    }

    AVLNode* _rightLeftCaseRotate(AVLNode* node) {
        // cout << node->data << " rl case" << endl;
    ...
    AVLNode* _insert_AVL(AVLNode* node, int value) {
        ...
        if(node == NULL){
            AVLNode* temp = _avl_createNode(value);
            // cout << "tadaa new node w ldtd " << *carry << endl;
            return temp;
        }
        if(value < node->data){
            // cout << "llleft " << *carry << endl;
            node->left = _insert_AVL(node->left,value);
        }
        else if(value > node->data){
            // cout << "rrright " << *carry << endl;
    ...
    void _sum(AVLNode *node, int l, int r){
        if (node) {
            _sum(node->left, l, r+1);
            mp[l - r] += node->data;
            _sum(node->right, l+1, r);
        }
    }
